...
export const Multiple: Story = {
  ...
  play: async ({canvasElement}) => {
    ...
    expect(input1).toHaveDisplayValue('12:00');
    ...
    expect(canvas.queryAllByText('Array of times instead of a single time value')).toHaveLength(1);
    ...
    expect(canvas.getByTestId('input-my-timefield[0]')).toHaveDisplayValue('');
    expect(canvas.queryByTestId('input-my-timefield[1]')).not.toBeInTheDocument();
  },
};
